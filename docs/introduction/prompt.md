...
本文为编译原理课程配套的提示工程专项教学内容，讲解了如何进行有效的 Prompt
Engineering，并在后续章节中提供 Task 1-4 的 Prompt Example。
...
**提示工程（Prompt Engineering）**
是一门聚焦提示词设计、开发与优化的新兴学科，核心是通过标准化的输入设计，引导大语言模型（Large
Language Model, 简称 LLM）输出符合预期的结果，是高效使用 LLM 的核心必备技能。
...
1. 提升输出质量：通过精准的提示设计，大幅提升 LLM 输出的准确性、相关性与可用性；
2. 拓展模型能力：通过引入专业领域知识、对接外部工具，突破 LLM 的原生能力边界；
3. 保障输出安全：通过规范的提示设计，规避 LLM 的幻觉、错误输出等问题，提升结果可靠性。
...
请以 C++ 代码格式输出完整的程序，将代码置于 `~~~cpp` 代码块中（以 `~~~cpp` 开始，以 `~~~` 结束），确保代码结构清晰、可编译运行，并在 main 函数中处理输入示例（如‘if x123 456’），输出每个 token 的类型和值（例如‘Keyword: if’, ‘Identifier: x123’, ‘Number: 456’）。输出应包含完整的 C++ 源代码，带有头文件、类定义和实现逻辑。
```
